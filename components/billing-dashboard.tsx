...
export default function BillingDashboard() {
  ...
  async function triggerPdf(invoiceId: string) {
    ...
    try {
      ...
      const pdfError = res.ok ? null : ((await res.json()) as { error?: string }).error ?? "Failed to generate PDF";
      await load();
  ...
  async function triggerReceipt(invoiceId: string) {
    ...
    try {
      ...
      const rcpError = res.ok ? null : ((await res.json()) as { error?: string }).error ?? "Failed to generate receipt";
      await load();
      if (rcpError) setError(rcpError);
    } finally {
      setBusySet((s) => { const n = new Set(s); n.delete(invoiceId + ":rcp"); return n; });
    }
  }

  async function clearReceipt(invoiceId: string) {
    if (!confirm("Delete this receipt PDF from Google Drive and allow regeneration?")) return;
    setBusySet((s) => new Set(s).add(invoiceId + ":rcp:del"));
    try {
      const res = await fetch(`/api/billing/invoices/${invoiceId}/receipt`, { method: "DELETE" });
      const err = res.ok ? null : ((await res.json()) as { error?: string }).error ?? "Failed to clear receipt";
      await load();
      if (err) setError(err);
    } finally {
      setBusySet((s) => { const n = new Set(s); n.delete(invoiceId + ":rcp:del"); return n; });
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <a href="/api/auth/google" className="text-xs text-zinc-400 hover:text-zinc-600 underline underline-offset-2">Connect Google</a>
      </div>
      <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      {error && (
        <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">
          {error.includes("invalid_grant") || error.includes("not configured") || error.includes("not connected")
            ? <>Google Drive session expired. <a href="/api/auth/google" className="underline font-medium">Reconnect Google</a> to generate PDFs and receipts.</>
            : error}
        </div>
      )}
                          ...
                          {isPaidOrPartial && (
                            row.receiptFileId ? (
                              <span className="inline-flex items-center gap-1.5">
                                ...
                                <button
                                  type="button"
                                  onClick={() => void clearReceipt(row.invoiceId!)}
                                  disabled={busySet.has(row.invoiceId! + ":rcp:del")}
                                  className="rounded px-1.5 py-1 text-xs text-zinc-400 hover:text-red-600 hover:bg-red-50 disabled:opacity-50"
                                  title="Delete receipt and regenerate"
                                >×</button>
                              </span>
                            ) : (
                              <button
